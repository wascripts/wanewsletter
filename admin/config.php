...
{
	trigger_error('Impossible de r�cup�rer la configuration du script', ERROR);
}
...
if( isset($_POST['submit']) )
{
	...
	if( !$error )
	{
		...
		{
			trigger_error('Impossible de mettre � jour la configuration', ERROR);
		}
		
		//
		// D�placement des fichiers joints dans le nouveau dossier de stockage s'il est chang�
		//
			...
			if( $result && ($res = opendir($nl_config['upload_path'])) )
			{
				while( $entry = readdir($res) )
				{
					if( $entry != '.' && $entry != '..' && is_file($nl_config['upload_path'] . $entry) && !ereg('^index\.html?$', $entry) )
					{
						...
						//
						// Suppression du fichier de l'ancien r�pertoire
						//
